...
namespace Blux
{
    public class Mesher
    {
        public static void MeshChunk(Chunk chunk, ref Mesh mesh)
        {
            ...
            int numVertices = numVoxels * 4 * 1;
            int numIndices = numVoxels * 2 * 3 * 1;
            ...
            NativeArray<float2> uvs = new(numVertices, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
        ...
        [BurstCompile]
        public static void MeshVoxel(byte block, int3 offset, ref int vertexCount, ref int indexCount, NativeArray<float3> vertices, NativeArray<float3> normals, NativeArray<float2> uvs, NativeArray<int> indices)
        {
            if (block == 0)
            {
                return;
            ...
            uvs[vertexCount] = new float2(0.0f, 0.0f);
            uvs[vertexCount + 1] = new float2(0.0f, 1.0f);
            uvs[vertexCount + 2] = new float2(1.0f, 1.0f);
            uvs[vertexCount + 3] = new float2(1.0f, 0.0f);
